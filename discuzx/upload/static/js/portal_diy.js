...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'Choose Style','value':''},{'key':'No Border and Borderless','value':'cl_block_bm'},{'key':'Style1','value':'xbs_1'},{'key':'Style2','value':'xbs xbs_2'},{'key':'Style3','value':'xbs xbs_3'},{'key':'Style4','value':'xbs xbs_4'},{'key':'Style5','value':'xbs xbs_5'},{'key':'Style6','value':'xbs xbs_6'},{'key':'Style7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'Choose Style','value':''},{'key':'No Border and Borderless','value':'cl_frame_bm'},{'key':'Borderless frames','value':'xfs xfs_nbd'},{'key':'Style1','value':'xfs xfs_1'},{'key':'Style2','value':'xfs xfs_2'},{'key':'Style3','value':'xfs xfs_3'},{'key':'Style4','value':'xfs xfs_4'},{'key':'Style5','value':'xfs xfs_5'}],
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'Repeat','value':'repeat'},{'key':'No Repeat','value':'no-repeat'},{'key':'Tile Repeat','value':'repeat-x'},{'key':'Vertical Repeat','value':'repeat-y'}];
		var stylearr = [{'key':'No Style','value':'none'},{'key':'Solid line','value':'solid'},{'key':'Dotted line','value':'dotted'},{'key':'Dashed line','value':'dashed'}];
		...
		table += '<li><label>up</label><select class="ps vm" id="bdtwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		...
		table += '<li class="bordera mtn"><label>Down</label><select class="ps vm" id="bdbwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
		...
		table += '<tr><th>Margins</th><td><div id="margindiv" class="'+ulclass+'"><span><label>up</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>Padding</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>up</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
		...
		table += '<tr><th>Background Images</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
	...
	openTitleEdit : function (e) {
		...
		var repeatarr = [{'key':'Repeat','value':'repeat'},{'key':'No Repeat','value':'no-repeat'},{'key':'Horizontal Repeat','value':'repeat-x'},{'key':'Vertical Repeat','value':'repeat-y'}];
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('Are you sure you want to delete it,Unrecoverable delete later')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('You have made ??changes，Please save and then do Export，Otherwise Export The data will not include your modifications in this。');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
			$('allupdate').innerHTML = 'Has been completed operation。';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'Total<span style="color:blue">'+this.blocksLen+'</span>modules,Now Update Article<span style="color:red">'+cur+'</span>,Completed<span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	blockForceUpdateBatch : function (blocks) {
		...
		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">Start Update...</div>','confirm','Update Module Data', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
		if (confirm('Are you sure you want to empty the data on the page where you DIY,Empty will not be restored after')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("Prompt：Framework not found，Please add the framework。");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">Press the Continue button will open a temporary data and DIY，<br />Press the Delete button to delete temporary data。</div>','confirm','Whether to continue the DIY store data？', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'Continue', 'Del');
...
spaceDiy.extend({
	...
	recover : function() {
		if (confirm('Are you sure you want to restore the previous version to save the results do？')) {
			drag.clearClose();
