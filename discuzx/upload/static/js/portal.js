...
function checkblockname(form) {
	...
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('stom content errors，HTML code：'+tag+' Tags do not match', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('HTML代码：'+tag+' Tags do not match', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
