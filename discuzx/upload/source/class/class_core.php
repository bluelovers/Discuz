...
class discuz_core {
	...
	function _init_input() {
		...
		// ±N isset ßÔ¨∞ !empty ¡◊ßK¶hæl™∫ htmlspecialchars
	...
	function reject_robot() {
		if (
			...
			&& (
				!defined('NOROBOT')
				// ÂÖÅË®±ÈÄöÈÅéÂÆöÁæ© NOROBOT = false ‰æÜÂÖÅË®± robots Ê™¢Á¥¢
				|| NOROBOT !== false
			)
			// bluelovers
		) {
	...
	function _init_user() {
		if($this->init_user) {
			...
			if ($discuz_uid) {
				/**
				 * ∞—¶“πÔ¿≥ function_member.php ; setloginstatus
				 * ≠◊ßÔ¨∞≠n®D¬sƒ˝æπ∞TÆß •≤∂∑≠n≤≈¶X ß_´hµ¯¶PµLÆƒ
				 *
				 * $auth
				 * 		0 => discuz_pw
				 * 		1 => discuz_uid
...
//			dexit(array($discuz_pw, $discuz_uid, $auth));
	...
	function _init_setting() {
		if($this->init_setting) {
			if(empty($this->var['setting'])) {
				/**
				 * ±N setting ±¿∞e®Ï≥Ã´e≠±
				 * ¡◊ßK¶PÆ…ßÛ∑sΩw¶sÆ…°Aπ¡∏’≈™®˙ setting ´o©|•º∏¸§J™∫∞›√D
				 **/
				if (empty($this->cachelist)) {
					// ≠◊•ø∑Ì $this->cachelist empty Æ… µL™k±¿§J∑s≠»
	...
	function _init_style($donot_define = 0) {
		...
		//BUG:¶π≥B¶]∏”¨O BUG ¶]¨∞ intval ¨O¶hælµL∑N∏q
		...
		/**
		 * ¶p™G•ÿ´e™∫ styleid §£µ•©Û ∫ÙØ∏πw≥]™∫ styleid
		 * ´h≈™®˙≠∑ÆÊ®√•B¬–ºg $this->var['style']
		 */
...
class db_mysql
{
	 ...
	 * pmlife  	int(10) unsigned  	NULL  	NO  	   	0  	   	select,insert,update,references  	PMÂ≠?¥ª???
	 */
...
class discuz_session {
	 ...
	 *
	 * @param $type = 0 | 1 | 2
	 * @param $type = 0 - ©“¶≥Ωu§W§Hº∆
	 * @param $type = 1 - ©“¶≥Ωu§W®œ•Œ™Ã§Hº∆
	 * @param $type = 2 - ©“¶≥Ωu§W¡Ù®≠§Hº∆
	 **/
