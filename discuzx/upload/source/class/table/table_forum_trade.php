 ...
 *
 *      $Id: table_forum_trade.php 27449 2012-02-01 05:32:35Z zhangguosheng $
 */
...
class table_forum_trade extends discuz_table
{
	...
	public function fetch_goods($tid, $pid, $orderby = '', $ascdesc = 'asc', $start = 0, $limit = 0) {
		...
		if($orderby) {
			$ordersql = " ORDER BY $orderby $ascdesc";
		}
	...
	public function fetch_all_statvars($fieldname, $limit = 10) {
		return DB::fetch_all("SELECT subject, tid, pid, seller, sellerid, SUM($fieldname) as $fieldname
	...
	public function fetch_all_for_search($digestltd, $fids, $topltd, $sqlsrch, $start = 0, $limit = 0) {
		return DB::fetch_all("SELECT tr.tid, tr.pid, t.closed FROM ".DB::table('forum_trade')." tr INNER JOIN ".DB::table('forum_thread')." t ON tr.tid=t.tid AND $digestltd t.fid IN ($fids) $topltd WHERE$sqlsrch ORDER BY tr.pid DESC".DB::limit($start, $limit));
	}
	public function fetch_all_for_space($wheresql, $ordersql, $count = 0, $start = 0, $limit = 0) {
		...
		return DB::fetch_all("SELECT t.* FROM ".DB::table('forum_trade')." t
				...
				WHERE $wheresql
				ORDER BY $ordersql ".DB::limit($start, $limit));
	}
}
