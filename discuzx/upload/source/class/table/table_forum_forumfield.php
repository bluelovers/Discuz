 ...
 *
 *      $Id: table_forum_forumfield.php 27449 2012-02-01 05:32:35Z zhangguosheng $
 */
...
class table_forum_forumfield extends discuz_table
{
	...
	public function fetch_all_by_fid($fids) {
		$fids = is_array($fids) ? array_map('dintval', $fids) : array(intval($fids));
		if(!empty($fids)) {
	...
	public function fetch_groupnum_by_founderuid($uid) {
		return DB::result_first("SELECT COUNT(*) FROM ".DB::table($this->_table)." WHERE founderuid='$uid'");
	}
	public function update_groupnum($fid, $num) {
		DB::query("UPDATE %t SET groupnum=groupnum+'$num' WHERE fid=%d", array('forum_forumfield', $fid));
	}
	public function update_membernum($fid, $num = 1) {
		DB::query("UPDATE %t SET membernum=membernum+'$num' WHERE fid=%d", array('forum_forumfield', $fid));
	}
	...
	public function check_moderator_for_uid($fid, $uid, $accessmasks = 0) {
		if($accessmasks) {
			...
			$accessadd2 = "LEFT JOIN ".DB::table('forum_access')." a ON a.uid='$uid' AND a.fid='$fid'";
		}
		return DB::fetch_first("SELECT ff.postperm, m.uid AS istargetmod $accessadd1
				FROM ".DB::table($this->_table)." ff
				$accessadd2
				LEFT JOIN ".DB::table('forum_moderator')." m ON m.fid='$fid' AND m.uid='$uid'
				WHERE ff.fid='$fid'");
	}
}
