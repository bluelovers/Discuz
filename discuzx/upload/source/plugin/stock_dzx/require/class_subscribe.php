...
class Subscribe
{
	...
	private function checkStock($user, $stock_id=0)
	{
		...
		$rs = DB::fetch_first("SELECT * FROM kfsm_apply WHERE sid='$stock_id'");
		if ( !$rs )
		{
		...
		else
		{
			...
			$buytime = DB::result_first("SELECT MAX(buytime) FROM kfsm_customer WHERE cid='$user[id]' AND sid='$stock_id'");
	...
	private function buyStock( $user, $stock_id, $price_buy, $num_buy )
	{
		...
		if ( $rs = $this->checkStock($user, $stock_id) )
		{
			...
			{
				$rs = DB::fetch_first("SELECT userid, stocknum, surplusnum FROM kfsm_apply WHERE sid='$stock_id'");
				...
				else
				{
					...
					else
					{
						...
						$rsc = DB::fetch_first("SELECT stocknum, averageprice, buytime FROM kfsm_customer WHERE cid='$user[id]' AND sid='$stock_id'");
						if ( !$rsc )
						{
							DB::query("INSERT INTO kfsm_customer (cid, username, sid, buyprice, averageprice, stocknum, buytime, ip) VALUES ('$user[id]', '$user[username]', '$stock_id', '$price_buy', '$price_buy', '$num_buy', '$_G[timestamp]', '$user[ip]')");
							DB::query("UPDATE kfsm_user SET capital=capital-{$worth}, capital_ava=capital_ava-{$worth}, stocksort=stocksort+1, todaybuy=todaybuy+{$num_buy}, lasttradetime='$_G[timestamp]' WHERE uid='{$user[id]}'");
						}
						else
						{
							...
							DB::query("UPDATE kfsm_customer SET stocknum=stocknum+{$num_buy}, buyprice='{$price_buy}', averageprice='{$avgprice}', buytime='{$_G[timestamp]}', ip='{$user[ip]}' WHERE cid='{$user[id]}' AND sid='$stock_id'");
							DB::query("UPDATE kfsm_user SET capital=capital-{$worth}, capital_ava=capital_ava-{$worth}, todaybuy=todaybuy+{$num_buy}, lasttradetime='{$_G['timestamp']}' WHERE uid='{$user[id]}'");
						}
						$kfsclass->calculatefund($user['id'], $stock_id);
						DB::query("UPDATE kfsm_apply SET surplusnum=surplusnum-{$num_buy}, capitalisation=capitalisation+{$worth} WHERE sid='$stock_id'");
						DB::query("UPDATE kfsm_sminfo SET todaybuy=todaybuy+{$num_buy}, todaytotal=todaytotal+{$worth}");
