...
class Tools
{
	...
	public function divide($num=1)
	{
		...
		{
			DB::query("UPDATE ".DB::table('kfsm_user')." SET capital=capital/{$num}, capital_ava=capital_ava/{$num} WHERE capital>{$num}");
		...
		if ( $confirm == '1' )
		{
			...
			if ( $stock_id == '-1' )
			{
				...
				while ( $rs = DB::fetch($query) )
				{
					...
					DB::query("UPDATE ".DB::table('kfsm_user')." SET capital=capital+{$stockvalue}, capital_ava=capital_ava+{$stockvalue}, asset=capital, stocksort='0', stockcost='0.00', stockvalue='0.00', todaysell=todaysell+{$rs['stocknum']} WHERE uid={$rs[cid]}");
				}
			...
			else
			{
				...
				{
					$stockvalue = $rs['stocknum'] * $rs['averageprice'];
					DB::query("UPDATE ".DB::table('kfsm_user')." SET capital=capital+{$stockvalue}, capital_ava=capital_ava+{$stockvalue}, todaysell=todaysell+{$rs['stocknum']} WHERE uid={$rs[cid]}");
	...
	public function distribute()
	{
		...
		if ( $usernum )
		{
			...
			else
			{
				...
				DB::query("UPDATE ".DB::table('kfsm_user')." SET capital=capital+{$perval}, capital_ava=capital_ava+{$perval} WHERE uid IN(" . pwImplode($touids) . ")");
