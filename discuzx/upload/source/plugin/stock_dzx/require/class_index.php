...
class Index
{
	public function subscribeCountDown()
	{
		...
		$rs = DB::fetch_first("SELECT sid, stockname, applytime FROM kfsm_apply WHERE state='1' AND surplusnum>0 ORDER BY aid LIMIT 1");
	...
	{
		$query = DB::query("SELECT aid, sid, stockname, stockprice, stocknum, surplusnum, applytime, issuetime FROM kfsm_apply WHERE state='1' ORDER BY aid DESC LIMIT 0,$num");
	...
	{
		$rs = DB::fetch_first("SELECT * FROM kfsm_sminfo");
		$ain_y = $rs['ain_y'];
		$ain_t = $rs['ain_t'];
