...
function kk_weibo_update($rel) {
	...
	if (empty($row)) DB::insert($table_stat_s, $data);
	else  DB::update($table_stat_s, $data, "uid={$rel}");
	...
	if (empty($row)) DB::insert($table_stat_s, $data);
	else  DB::update($table_stat_s, $data, "uid={$uid}");
}
//-------------------------------------------------------------------------------------------------
$action = $_G['gp_action'];
$uid = $_G['uid'];
$uid_rel = (int)$_G['gp_rel'];
kk_weibo_check();

$table = DB::table('kk_weibo');
$table_stat_s = 'kk_weibo_stat';
$table_stat = DB::table($table_stat_s);
$description = "";
if ($action == 'add') {
...
showmessage($description, dreferer(), array(), array('showmsg' => 1, 'showdialog' => 1, 'locationtime' => 3));
