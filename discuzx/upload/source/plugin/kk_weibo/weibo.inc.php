...
$view = $_GET['view'];
if (empty($view)) $view = 'attention';
$uid_rel = (int)$_GET['rel'];
if ($uid_rel <= 0) $uid_rel = $_G['uid'];
...
$table_weibo = DB::table('kk_weibo');
if ($view == 'attention') {
	$theurl = "/plugin.php?id=kk_weibo:weibo&view=attention&rel={$uid_rel}";
	$count = DB::fetch_first("select count(*) as t_count from {$table_weibo} where uid={$uid_rel}");
	$count = $count['t_count'];
	if (!empty($count)) {
	...
	if ($view == 'fans') {
		...
		$count = DB::fetch_first("select count(*) as t_count from {$table_weibo} where uid_rel={$uid_rel}");
		$count = $count['t_count'];
		if (!empty($count)) {
