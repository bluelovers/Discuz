...
function share_del() {
    ...
    if(!submitcheck("share_del")){
        include template("wb_share_dzx:share_del");
    }else{
    DB::delete("share_keys", "apiname='{$apiname}' and uid='{$_G['uid']}'");
    showmessage('wb_share_dzx:deletesuccess', dreferer(), array(), array('showdialog'=>1, 'showmsg' => true, 'closetime' => true));
    }
}
...
function share_sendmsg() {
    global $_G;
    if (submitcheck("sendmsg")) {
        //判断是否为空
        if (empty($_POST['msg'])) {
            showmessage("wb_share_dzx:msgisempty");
        }
        //判断是否绑定微博
        checkbind();
        require_once 'class/Share.class.php';
