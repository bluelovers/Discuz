...
$vkscp = DB::fetch_first("SELECT votes,uids FROM " . DB::table('nds_votekick') .
	" WHERE tid = '" . $tid . "'");
$vkusers = '';
$vkuids = '0';
$votes = 0;
if ($vkscp['votes']) {
	...
	foreach ($vkuids as $vkuid) {
		...
		$vkusers .= "<a href=\"home.php?mod=space&uid=$vkuid\" target=\"_blank\">" . $vkuser['username'] .
			"</a>&nbsp;&nbsp;";
	}
}
...
$vkthread = DB::fetch_first("SELECT fid,authorid,subject,dateline FROM " . DB::table
	('forum_thread') . " WHERE tid = '" . $tid . "'");
if ($_G['cache']['plugin']['nds_votekick']['vtexpired'] > 0 && ($_G['timestamp'] -
	$vkthread['dateline']) > $_G['cache']['plugin']['nds_votekick']['vtexpired'] *
	24 * 60 * 60) {
...
if (submitcheck('votekicksubmit')) {
	if ($votes) {
		DB::query("UPDATE " . DB::table('nds_votekick') .
			" SET votes = votes + 1,uids = CONCAT(uids,'\t','" . $_G['uid'] .
			"')  WHERE tid='" . $tid . "'");
	} else {

		DB::query("INSERT INTO " . DB::table('nds_votekick') .
			" (tid, votes, uids) VALUES ('$tid','1' ,'$_G[uid]')");
	}

	$notevars = array('url1' => "forum.php?mod=viewthread&tid=" . $tid, 'subject1' =>
		$vkthread['subject'], 'vkuser' => "<a href=\"home.php?mod=space&uid=$_G[uid]\">" .
		$_G['member']['username'] . "</a>", 'reason' => dhtmlspecialchars(dstripslashes
		(cutstr(trim($_POST['description']), 20))));

	$mistoadminid = $_G['cache']['plugin']['nds_votekick']['votekickadminid'] > 1 ?
		$_G['cache']['plugin']['nds_votekick']['votekickadminid'] : $_G['config']['admincp']['founder'];
	if ($_G['cache']['plugin']['nds_votekick']['votetoauthor']) {
		notification_add($vkthread['authorid'], 'votekick', lang('plugin/nds_votekick',
			'notice1'), $notevars, '', 1);
	}
	if ($_G['cache']['plugin']['nds_votekick']['votetoadmin']) {
		notification_add($mistoadminid, 'votekick', lang('plugin/nds_votekick',
			'notice2'), $notevars, '', 1);
	}
	...
	if ($votes + 1 >= $lvotes) {
		switch ($treatmod) {
			case 1:
				DB::query("UPDATE " . DB::table('forum_thread') .
					" SET displayorder = -2  WHERE tid='" . $tid . "'");
				updatemoderate('tid', $tid);
				$opmode = lang('plugin/nds_votekick', 'vkop1');
				break;
			case 2:
				DB::query("UPDATE " . DB::table('forum_thread') .
					" SET displayorder = -1  WHERE tid='" . $tid . "'");
			...
			case 3:
				...
				DB::query("UPDATE " . DB::table('common_member') .
					" SET adminid = -1,groupid = 4  WHERE uid='" . $vkthread['authorid'] . "'");
			...
			case 5:
				...
				updatemoderate('tid', $tid);
				DB::query("UPDATE " . DB::table('common_member') .
					" SET adminid = -1,groupid = 5  WHERE uid='" . $vkthread['authorid'] . "'");
			...
			case 8:
				DB::query("UPDATE " . DB::table('forum_thread') .
					" SET displayorder = -1  WHERE authorid ='" . $vkthread['authorid'] . "'");
				DB::query("UPDATE " . DB::table('common_member') .
					" SET adminid = -1,groupid = 5  WHERE uid='" . $vkthread['authorid'] . "'");
		...
		$notevars2 = array('url1' => "forum.php?mod=viewthread&tid=" . $tid, 'subject1' =>
			$vkthread['subject'], 'opmod' => $opmode, 'vkuser' => "<a href=\"home.php?mod=space&uid=$_G[uid]\">" .
			$_G['member']['username'] . "</a>", 'vkcount' => $lvotes, );
	...
	showmessage('nds_votekick:vkok', dreferer(), array(), array('showdialog' => true,
		'closetime' => true));
