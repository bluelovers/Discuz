...
$authorlz = base64_decode(getcookie('sco_amucallme_lz'.$tp));
...
$gids = (array)unserialize($_G['cache']['plugin']['sco_amucallme']['gids']);
...
if(file_exists('./data/plugindata/sco_amucallme.data.php')){
	...
	if($cmcost['cost']>=1)$costshow = lang('plugin/sco_amucallme','sf',array('cost' => $cname));
}
include template('sco_amucallme:callme');
