...
class plugin_sco_amucallme extends _sco_dx_plugin {
	...
	function sco_amucallme_output($a){
		...
		if($_G['uid'] && in_array($_G['fid'],$this->fids)){
			...
			$sendmsg = lang('plugin/sco_amucallme','sendmsg',array('username' => $_G['username'],'url' => $url,'reply' => $reply,'message' => $msg));
			$cmcost=array();
			if(file_exists('./data/plugindata/sco_amucallme.data.php')){
...
class plugin_sco_amucallme_forum extends plugin_sco_amucallme {
	...
	function viewthread_postfooter_output(){
		...
		if($_G['uid']){
			foreach ($postlist as $value){
				...
				if($author <> $username){
					$authors[] = $author;
					$shows[]= "<a class='add_callme' href='javascript:;' onclick='seditor_insertunit(\"fastpost\", \"[@]".$author."[/@]\");window.scrollTo(0,99999);'>".lang('plugin/sco_amucallme','callme')."</a>";
				}else{$shows[]= '';}
			}
			if(array_unique($authors)){
				$authors = serialize(array_unique($authors));
				dsetcookie('sco_amucallme_'.$this->tp, base64_encode($authors), 600);
				dsetcookie('sco_amucallme_lz'.$this->tp, base64_encode($_G['forum_thread']['author']), 600);
			}
			//var_dump($shows);
		}
		return (array)$shows;
	...
	function viewthread_fastpost_ctrl_extra() {
		...
		if($_G['uid']){
			...
			$str = '<div class="sco_amucallme_tag"><a href="plugin.php?id=sco_amucallme:callme&adds=fastpostmessage&tp='.$this->tp.'" title="'.lang("plugin/sco_amucallme","callme").'" onclick="showWindow(\'sco_amucallme_add\', this.href);">'.lang("plugin/sco_amucallme","callme").'</a></div>';
			$css = '<style>';
			$css .= '.sco_amucallme_tag a {background:url("source/plugin/sco_amucallme/images/a1.png") no-repeat scroll 0 0 transparent;float:left;height:20px;line-height:20px;margin:4px 4px 0px 4px;overflow:hidden;text-indent:-9999px;width:20px;}';
			$css .= '.add_callme{background:url(source/plugin/sco_amucallme/images/a1.png) no-repeat 0 50%;}</style>';
		}
		return $str.$css;
	}
	...
	function post_amucallme(){
		...
		if($_G['uid'] && in_array($_G['fid'],$this->fids)){
			...
			dsetcookie('sco_amucallme_ed', base64_encode($authorsd), 600);
			if($pp["username"]){
