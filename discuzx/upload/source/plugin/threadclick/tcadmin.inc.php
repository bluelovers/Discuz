...
if(!submitcheck('clicksubmit')) {
	...
	showtips(lang('plugin/threadclick', 'click_edit_tips'));
	showformheader('plugins&operation=config&do='.$_G['gp_do']);
	showtableheader();
...
EOF;
	...
	while($click = DB::fetch($query)) {
		...
		$click['idtype'] = cplang(lang('plugin/threadclick', 'click_edit_'.$click['idtype']));
		showtablerow('', array('class="td25"', 'class="td28"', 'class="td25"', 'class="td25"', '', '', '', 'class="td23"', 'class="td25"'), array(
	...
	updatecache('click');
	cpmsg('click_edit_succeed', 'action=plugins&operation=config&do='.$_G['gp_do'], 'succeed');
}
