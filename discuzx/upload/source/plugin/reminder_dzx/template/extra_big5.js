	...
	for(var one in jsndata) {
		switch(one) {
			...
			case 'newpm':
				...
				for(var two in jsndata[one]) {
					newreminder = newreminder+'_'+two
					list.push('<li><a href="home.php?mod=space&uid='+jsndata[one][two].lastauthorid+'" target="_blank">'+jsndata[one][two].lastauthor+'</a> 說 :<span id="p_gpmid_'+jsndata[one][two].pmid+'">'+jsndata[one][two].lastsummary+'</span> &nbsp; <a href="home.php?mod=spacecp&amp;ac=pm&amp;op=showmsg&amp;handlekey=showmsg_'+jsndata[one][two].lastauthorid+'&amp;touid='+jsndata[one][two].lastauthorid+'&amp;pmid=0&amp;daterange=2" id="a_sendpm_'+jsndata[one][two].lastauthorid+'" onclick="showWindow(\'showMsgBox\', this.href, \'get\', 0)">發送消息</a></li>');
				}
				break;
			case 'newthread':
				...
				for(var two in jsndata[one]) {
					newreminder = newreminder+'_'+two
					list.push('</li>' + '本版有新的回復主題： <a href="forum.php?mod=viewthread&tid='+jsndata[one][two].tid+'" target="_blank">' + jsndata[one][two].subject + '</a></li>');
				}
				break;
		}
	}
