	...
	for(var one in jsndata) {
		switch(one) {
			...
			case 'newpm':
				...
				for(var two in jsndata[one]) {
					newreminder = newreminder+'_'+two
					list.push('<li><a href="home.php?mod=space&uid='+jsndata[one][two].lastauthorid+'" target="_blank">'+jsndata[one][two].lastauthor+'</a> 說 :<span id="p_gpmid_'+jsndata[one][two].pmid+'">'+jsndata[one][two].lastsummary+'</span> &nbsp; <a href="home.php?mod=space&do=pm&subop=view&touid='+jsndata[one][two].lastauthorid+'#last" id="pmlist_'+jsndata[one][two].pmid+'_a" target="_blank">回復</a></li>');
				}
				break;
			case 'newthread':
