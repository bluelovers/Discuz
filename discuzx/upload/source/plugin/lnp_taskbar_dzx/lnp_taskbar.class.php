...
class plugin_lnp_taskbar_dzx {
	...
	function global_footer() {
		...
		if(getcookie('lnp_topen') == 1 || (getcookie('lnp_topen') != -1 && $lnpdata['open_bar'])) {
			$disopen = '';
	...
	function _getOnlineFriend() {
		...
		if($_G['uid'] > 0 && $user['feedfriend']) {
			$olnum = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('common_session')." WHERE uid IN ({$user['feedfriend']}) AND invisible='0'"), 0);
			$olnum = $olnum ? $olnum : 0;
		}
		if($olnum && $user['feedfriend']) {
			...
			while($value = DB::fetch($query)) {
				$liststr .= <<<EOT
				<li><img id="lnp_poke_u{$value['uid']}" onclick="showWindow(this.id, 'home.php?mod=spacecp&amp;ac=poke&amp;op=send&uid={$value['uid']}', 'get', 0);" class="lnp_olicon" title="点击图片和{$value['username']}打招呼!" src="{$staticurl}image/feed/poke.gif" /><a title="点击这里和 {$value['username']} 聊天" href="home.php?mod=space&amp;uid={$value['uid']}" onclick="showWindow('showMsgBox{$value['uid']}', 'home.php?mod=spacecp&amp;ac=pm&amp;op=showmsg&amp;handlekey=showmsg_{$value['uid']}&amp;touid={$value['uid']}&pmid=0&daterange=2', 'get', 0)" ><img alt="{$value['username']}" src="{$staticurl}image/common/user_online.gif">{$value['username']}</a></li>
EOT;
			}
		}
