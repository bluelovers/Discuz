/*
	...
	$Id: common.js 620 2010-09-09 03:23:24Z yexinhao $
*/
...
//note loadcss
if(BROWSER.ie) {
	document.getElementsByTagName('head')[0].parentNode.className = 'ie_all ie' + parseInt(BROWSER.ie);
}
...
function setMenuPosition(showid, menuid, pos) {
	function checkmenuobj(showObj, menuObj) {
		while((showObj = showObj.offsetParent) != null) {
			if(showObj.style.position == 'relative') {
				return 0;
			}
		}
	...
	if(showObj) {
		sxy = fetchOffset(showObj, !BROWSER.ie || BROWSER.ie >= 8 ? 0 : checkmenuobj(showObj, menuObj));
		sx = sxy['left'];
		sy = sxy['top'];
...
function AC_GetArgs(args, classid, mimeType) {
	...
	for (var i = 0; i < args.length; i = i + 2){
		...
		switch (currArg){
			...
			case "id":ret.objAttrs[args[i]] = args[i+1];ret.embedAttrs[args[i]] = args[i+1];break;
...
function simulateSelect(selectId, widthvalue) {
	...
	$(selectId + '_ctrl').onclick = function(e) {
		...
		showMenu({'ctrlid':(selectId == 'loginfield' ? 'account' : selectId + '_ctrl'),'menuid':selectId + '_ctrl_menu','evt':'click','pos':'43','maxh':maxh});
		doane(e);
	};
...
function loadimgsize(imgurl, editor) {
	...
	s.loadCheck = function () {
		if(s.img.complete) {
			$(editor + '_image_submit').disabled = false;
			$(editor + '_image_param_2').value = s.img.width ? s.img.width : '';
			$(editor + '_image_param_3').value = s.img.height ? s.img.height : '';
...
function seditor_insertunit(key, text, textend, moveend, selappend) {
	...
	selappend = isUndefined(selappend) ? 1 : 0;
...
if(typeof IN_ADMINCP == 'undefined') {
	...
	_attachEvent(window, 'load', initCard, document);
}
...
function extstyle(css) {
	$('extstyle_css').href = css ? css + '/style.css' : '';
	currentextstyle = css;
...
function createPalette(colorid,id) {
	var iframe = "<iframe name=\"c"+colorid+"_frame\" src=\"\" frameborder=\"0\" width=\"199\" height=\"148\" scrolling=\"no\"></iframe>";
	if (!$("c"+colorid+"_menu")) {
...
function showCard(obj) {
	USERCARDST = setTimeout(function() {ajaxmenu(obj, 500, 1, 2);}, 250);
}

function initCard() {
	var a = document.body.getElementsByTagName('a');
	for(var i = 0;i < a.length;i++){
		if(a[i].getAttribute('c') && a[i].getAttribute('c') != 0 || a[i].href.match(/home.php\?mod=space&(amp;)?u[^&]+$/)) {
			a[i].onmouseover = function() {showCard(this);}
...
function lsShowmore() {
	...
	var ppos = BROWSER.ie && BROWSER.ie < 8 ? fetchOffset($('um')) : {'left':0,'top':0};
	var objw = $('ls_password').offsetWidth;
	var objh = $('ls_password').offsetHeight;
	$('ls_more').style.position = 'absolute';
	$('ls_more').style.left = (objpos['left'] + objw - 173 - ppos['left']) + 'px';
	$('ls_more').style.top = (objpos['top'] + objh - ppos['top']) + 'px';
