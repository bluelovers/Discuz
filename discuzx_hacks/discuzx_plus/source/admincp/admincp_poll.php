...
if($operation == 'setting') {
	...
	if($operation == 'create') {
		...
		if(!submitcheck('managesubmit')) {
			...
			//debug 1 cookie, 2 用户名, 3 ip, 4 share object
	...
	//选项设置
		...
		if($itemid && empty($_G['gp_ip'])) {
			...
			if(!empty($itemcountlist)) {
				...
				showsubmit('', '', '', '<a href="'.ADMINSCRIPT.'?action=poll&operation=counter&countpollsubmit=yes&formid='.$itemid.'">还没有进行过数据分析，点击进行分析</a>');
			}
			...
			if(!empty($choicelist)) {
				...
				showsubmit('', '', '', '<a href="'.ADMINSCRIPT.'?action=poll&operation=counter&countoptionsubmit=yes&formid='.$itemid.'&formip='.$_G['gp_ip'].'">还没有进行过数据分析，点击进行分析</a>');
			}
