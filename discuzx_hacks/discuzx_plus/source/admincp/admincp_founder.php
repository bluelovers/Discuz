 ...
 *
 *      $Id: admincp_founder.php 646 2010-09-13 03:37:40Z yexinhao $
 */
	...
	if($do == 'group') {
		...
		if(!$id) {
			...
			if(!submitcheck('submit')) {
				...
				if(!empty($_G['gp_newcpgroupname'])) {
					if(in_array($_G['gp_newcpgroupname'], array(cplang('founder_master'), cplang('founder_admin'))) || DB::result_first("SELECT count(*) FROM ".DB::table('common_admincp_group')." WHERE cpgroupname='$_G[gp_newcpgroupname]'")) {
		...
		//note 权限编辑
			...
			if(!submitcheck('submit')) {
				...
				showsubmenu('menu_global_perm', array(array()), $grouplist, array('group' => $cpgroupname));
				...
				foreach($data['cats'] as $topkey) {
					...
					$row .= '</td></tr>';
					if($topkey != 'setting') {
						showtitle('<label><input class="checkbox" type="checkbox" onclick="permcheckall(this, \'perms_'.$topkey.'\')" '.($checkedall ? 'checked="checked" ' : '').'/> '.$data['menuname'][$topkey].'</label>');
					} else {
	...
	//note 团队成员
		...
		if(!$id) {
			if(!submitcheck('submit')) {
				...
				if(!empty($_G['gp_newcpusername'])) {
					...
					DB::update('common_member', array('allowadmincp' => 1, 'adminid' => 1, 'groupid' => 1), "uid='$newcpuid'");
				}
				if(!empty($_G['gp_delete'])) {
					DB::delete('common_admincp_member', 'uid IN ('.dimplode($_G['gp_delete']).')');
					DB::update('common_member', array('allowadmincp' => 0, 'adminid' => 0, 'groupid' => 2), 'uid IN ('.dimplode($_G['gp_delete']).')');
				}
		...
		//note 权限编辑
			...
			if(!submitcheck('submit')) {
				...
				showpermstyle();
				showsubmenu('menu_global_perm', array(array()), '', array('username' => $username));
				...
				foreach($data['cats'] as $topkey) {
					...
					$row .= '</td></tr>';
					if($topkey != 'setting') {
						showtitle('<input class="checkbox" type="checkbox" onclick="permcheckall(this, \'perms_'.$topkey.'\')" '.($checkedall ? 'checked="checked" ' : '').'/> '.$data['menuname'][$topkey].'</label>');
					} else {
						showtitle('founder_perm_setting');
					}
					echo $row;
		...
		if(!submitcheck('submit')) {
			...
			showsubmenu('menu_global_permgrouplist', array(array()), '', array('perm' => cplang($gset[0])));
...
function getactionarray() {
	...
	return array('actions' => $actionarray, 'cats' => $actioncat, 'menuname' => $topmenu);
}
