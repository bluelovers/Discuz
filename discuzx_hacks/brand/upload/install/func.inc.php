...
function check_db($dbhost, $dbuser, $dbpw, $dbname, $tablepre) {
	...
	if(!@mysql_connect($dbhost, $dbuser, $dbpw)) {
		...
		if(strcmp($mysql_version, '4.1') < 0) {
			show_msg('mysqld_required_version', $error, 0);
		} elseif($query = mysql_query("SHOW TABLES FROM $dbname")) {
...
function env_check(&$env_items) {
	foreach($env_items as $key => $item) {
		...
		} elseif(in_array($key, array('register_globals', 'short_open_tag', 'output_buffering'))) {
			$env_items[$key]['current'] = intval(@ini_get($key));
			if($env_items[$key]['current']>0) {
				$env_items[$key]['current']='ON';
			} else {
				$env_items[$key]['current']='OFF';
			}
		} elseif(isset($item['c'])) {
			$env_items[$key]['current'] = constant($item['c']);
		} elseif(isset($item['i'])) {
			$env_items[$key]['current'] = @ini_get($item['i']);
		}
...
function show_footer($quit = true) {
	...
	echo <<<EOT
		<div class="footer">&copy;2009 - 2010 <a href="http://www.comsenz.com/">Comsenz</a> Inc.</div>
	</div>
</div>
</body>
</html>
...
function config_edit() {
	...
	$configfile = preg_replace("/[$]_SC\[[\"']dbhost[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['dbhost'] = '$dbhost';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']dbuser[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['dbuser'] = '$dbuser';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']dbpw[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['dbpw'] = '$dbpw';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']dbname[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['dbname'] = '$dbname';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']adminemail[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['adminemail'] = '$adminemail';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']tablepre[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['tablepre'] = '$tablepre';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']cookiepre[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['cookiepre'] = '".random(3)."_';", $configfile);
	$configfile = preg_replace("/[$]_SC\[[\"']founder[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['founder'] = '$uid';", $configfile);

	@file_put_contents(CONFIG, $configfile);
}

function config_edit_siteurl() {
	extract($GLOBALS, EXTR_SKIP);
	global $_G, $siteurl;
	...
	$configfile = trim($configfile);
	$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
	$configfile = preg_replace("/[$]_SC\[[\"']siteurl[\"']\]\s*\=\s*[\"'].*?[\"'];/is", "\$_SC['siteurl'] = '$siteurl';", $configfile);
...
function runquery($sql) {
	global $_G, $lang, $tablepre, $db;

	if(!isset($sql) || empty($sql)) return;
	$sql = str_replace(array('`'.ORIG_TABLEPRE, ' '.ORIG_TABLEPRE, "\r"), array('`'.$tablepre, ' '.$tablepre, "\n"), $sql);
	$ret = array();
	$num = 0;
	foreach(explode(";\n", trim($sql)) as $query) {
		$ret[$num] = '';
...
function install_testdata($username, $uid) {
	global $_G, $db, $tablepre, $testdatacontent;
//
}