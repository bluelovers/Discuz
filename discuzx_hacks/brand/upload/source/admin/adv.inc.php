...
$root = '<a href="?action=adv">'.lang('adv_admin').'</a>';
...
if($operation == 'ad') {
	...
	if(!submitcheck('advsubmit')) {
		...
		if(!$type) {
			$query = DB::query("SELECT * FROM ".DB::table('advertisement_custom'));
			$customadv = array();
		...
		$query = DB::query("SELECT * FROM ".DB::table('advertisement')." WHERE 1 $conditions ORDER BY available DESC, $order_by LIMIT $start_limit, $advppp");
		$typenames = array();
		while($adv = DB::fetch($query)) {
			...
			showtablerow('', array('class="td25"', 'class="td25"', 'class="td25"'), array(
				...
				"<input type=\"text\" class=\"txt\" size=\"15\" name=\"titlenew[$adv[advid]]\" value=\"".htmlspecialchars_decode($adv['title'])."\">",
				...
				$adv['starttime'] ? gmdate($adv['starttime'], 'd') : $lang['unlimited'],
				$adv['endtime'] ? gmdate($adv['endtime'], 'd') : $lang['unlimited'],
...
<input type="button" class="btn" name="searchsubmit" value="'.cplang('search').'" onclick="if(this.form.title.value==\''.cplang('adv_inputtitle').'\'){this.form.title.value=\'\'}location.href=\''.ADMINSCRIPT.'?action=adv&operation=ad'.($type ? '&type='.rawurlencode($type) : '').'&title=\'+this.form.title.value+\'&starttime=\'+this.form.starttime.value+\'&endtime=\'+this.form.endtime.value+\'&orderby=\'+this.form.orderby.value;"> &nbsp;
		...
		if($advids) {
			$query = DB::query("SELECT parameters FROM ".DB::table('advertisement')." WHERE advid IN ($advids)");
			while($adv = DB::fetch($query)) {
	...
	if(!submitcheck('advsubmit')) {
		...
		echo '<script type="text/javascript" src="static/js/forum_calendar.js"></script>'.
			...
			(file_exists(B_ROOT.'./static/image/admin/'.$type.'.gif') ? '<img src="static/image/admin/'.$type.'.gif" />' : '').
	...
	if($do == 'add') {
		...
		if($addcustom) {
			if(!($customid = DB::result_first("SELECT id FROM ".DB::table('advertisement_custom')." WHERE name='$addcustom'"))) {
				DB::insert('advertisement_custom', array('name' => $addcustom));
