 ...
 *
 *      $Id: admincp_exttools.php 2279 2010-09-28 06:40:17Z songlixin $
 */
...
@define('TOOLS_VERSION', '1.4M');
@define('TOOLS_RELEASE', '20100716');
...
EOT;
	...
	if($_G['gp_'.$cp.'_submit'] || $step>0){
		if($cp == 'synusername'){
			...
			$query = DB::query("SELECT uid FROM ".DB::table('common_member')." WHERE uid > $start LIMIT $limit");
		...
		if($clearpms) {
			...
			cpmsg($rows,ADMINSCRIPT."?action=exttools&operation=$operation&do=tools&cp=pm");
		}
		...
		$pmss = DB::query("SELECT * FROM ".UC_DBTABLEPRE."pms $sqlplus GROUP BY dateline, msgtoid ORDER BY dateline DESC LIMIT $startlimit,$ppp");
